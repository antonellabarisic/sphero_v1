...
class SpheroNode(object):
	...
	def __init__(self,default_update_rate=50.0):
		...
		self._num_of_sphero=rospy.get_param(self._node_name+"/num")
	...
	def _init_pubsub(self):
		self.heading_sub = rospy.Subscriber('set_heading_'+self._num_of_sphero, Float32, self.set_heading, queue_size = 1)
		self.cmd_vel_sub = rospy.Subscriber('cmd_vel_'+self._num_of_sphero, Twist, self.cmd_vel, queue_size = 1)
		self.color_sub = rospy.Subscriber('set_color_'+self._num_of_sphero, ColorRGBA, self.set_color, queue_size = 1)
		self.stabilization_sub = rospy.Subscriber('disable_stabilization_'+self._num_of_sphero, Bool, self.set_stabilization, queue_size = 1)
		self.angular_velocity_sub = rospy.Subscriber('set_angular_velocity_'+self._num_of_sphero, Float32, self.set_angular_velocity, queue_size = 1)
